...
The policies (Forge, Arc, Sentry, Bible) are the instruments. This file is the
...
Project stages run once per project and re-open only when stale.
...
| P2 | `constitution` | Forge `discover` | yes | constitution approved; Forge re-validated to `ready` |
...
The owner may waive a stage. ShowRunner never proposes a waiver.
...
Stage: <stage> (<n>/15) - <in progress | awaiting owner | blocked | parked>
...
## 7. Batching Owner Gates

When consecutive owner gates are ready together (for example design output
and the `arc-ready` handoff), ShowRunner may present them in one message.
Each gate still gets its own ledger row, and the owner's reply must address
each one. Silence on a gate is not approval.
...
Sentry monthly cycles (when enabled), and stale accepted risks before new
work.
